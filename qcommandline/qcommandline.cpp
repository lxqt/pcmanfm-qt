...
QCommandLine::parse()
{
  ...
  Q_FOREACH (QCommandLineConfigEntry entry, d->config) {
  ...
  for (int i = 1; i < args.size(); ++i) {
    ...
    if (param) {
      if (!params.size()) {
	Q_EMIT parseError(tr("Unknown param: %1").arg(arg));
	return false;
      }
	...
	entry.flags = (QCommandLine::Flags) (entry.flags | QCommandLine::Optional);
      ...
      Q_EMIT paramFound(entry.longName, arg);
	...
	key = arg;
      ...
      if (c.find(key) == c.end()) {
	Q_EMIT parseError(tr("Unknown option: %1").arg(key));
	return false;
      }
	...
	if (stay) {
	  Q_EMIT parseError(tr("Option %1 need a value").arg(key));
	  return false;
	}
  ...
  Q_FOREACH (QCommandLineConfigEntry entry, params) {
    if (entry.flags & QCommandLine::Mandatory) {
      Q_EMIT parseError(tr("Param %1 is mandatory").arg(entry.longName));
      return false;
    }
  }

  Q_FOREACH (QCommandLineConfigEntry entry, conf.values()) {
    if (entry.flags & QCommandLine::Mandatory) {
      QString type;
	...
	type = tr("Option");
      ...
      Q_EMIT parseError(tr("%1 %2 is mandatory").arg(type).arg(entry.longName));
      return false;
    }
  }

  Q_FOREACH (QString key, switchs) {
    for (int i = 0; i < switchsFound[key]; i++) {
      if (d->help && key == helpEntry.longName)
	showHelp();
      if (d->version && key == versionEntry.longName)
	showVersion();
      Q_EMIT switchFound(key);
    }
  }

  Q_FOREACH (QString key, options) {
    Q_FOREACH (QString opt, optionsFound[key])
      Q_EMIT optionFound(key, opt);
  }  return true;
}
